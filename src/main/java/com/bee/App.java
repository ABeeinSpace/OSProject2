...
public class App {
    ...
    public static void assertTest(boolean test, String errorMessage,
            int pointsSoFar) {
    ...
    public static void main(String[] args) {
        ...
        assertTest(tiny.toString().contains("Hi"),
                "I don't see the added element in the toString!",
                expectedPoints);
        assertTest(!strings.toString().contains("Hi"),
                "Why does the toString include the string 'Hi'?  It shouldn't.",
                expectedPoints);
        assertTest(ints.toString().contains("1337"),
                "I don't see the added element in the toString!",
                expectedPoints);
        ...
        assertTest(
                lastInt == 1337,
                "Remove doesn't return the correct element on a queue of integers.",
                expectedPoints);
        assertTest(
                !ints.toString().contains("1337"),
                "It doesn't look like remove removes the element from the queue!",
                expectedPoints);
        String lastString = tiny.remove();
        assertTest(
                lastString.equals("Hi"),
                "Remove doesn't return the correct element on a queue of Strings.",
                expectedPoints);
        strings.add("1");
        strings.add("2");
        ...
        assertTest(ints.getNumElements() == 0,
                "getNumElements doesn't work on an empty list.", expectedPoints);
        tiny.add("Hello");
        assertTest(tiny.getNumElements() == 1,
                "getNumElements doesn't work on a list with one  element.",
                expectedPoints);
        strings.add("mankey");
        strings.add("primeape");
        assertTest(
                strings.getNumElements() == 2,
                "getNumElements doesn't seem to work on a list with two elements.",
                expectedPoints);
        for (int i = 0; i < 13; i++) {
            ints.add(i);
            assertTest(ints.getNumElements() == (i + 1),
                    "getNumElements isn't working when there are " + i +
                            " elements.",
                    expectedPoints);
        }
        ...
        assertTest(ints.getFreeSpace() == 13,
                "getFreeSpace doesn't work on an empty list.", expectedPoints);
        tiny.add("Hello");
        assertTest(tiny.getFreeSpace() == 0,
                "getFreeSpace doesn't work on a list with one  element.",
                expectedPoints);
        strings.add("mankey");
        strings.add("primeape");
        assertTest(strings.getFreeSpace() == 8,
                "getFreeSpace doesn't seem to work on a list with two elements.",
                expectedPoints);
        for (int i = 0; i < 13; i++) {
            ints.add(i);
            assertTest(ints.getFreeSpace() == (13 - i - 1),
                    "getFreeSpace isn't working when there are " + i +
                            " elements.",
                    expectedPoints);
        }
        ...
        for (int i = 0; i < 13; i++) {
            assertTest(ints.remove() == i,
                    "The queue doesn't maintain the FIFO-policy.", expectedPoints);
        }
        ints.add(5);
        ints.add(7);
        ints.add(9);
        assertTest(ints.remove() == 5,
                "The queue doesn't maintain the FIFO-policy.", expectedPoints);
        ints.add(11);
        ints.add(13);
        assertTest(ints.remove() == 7,
                "The queue doesn't maintain the FIFO-policy.", expectedPoints);
        assertTest(ints.remove() == 9,
                "The queue doesn't maintain the FIFO-policy.", expectedPoints);
        assertTest(ints.remove() == 11,
                "The queue doesn't maintain the FIFO-policy.", expectedPoints);
        assertTest(ints.remove() == 13,
                "The queue doesn't maintain the FIFO-policy.", expectedPoints);
        ...
        assertTest(flags.size() == 1, "Remove doesn't block correctly.",
                expectedPoints);
        ints.add(1);
        ...
        for (int i = 0; i < 10; i++) {
            ...
            assertTest(flags.size() == 1,
                    "Remove doesn't block correctly! i=" + i + " ints: " + ints +
                            " flags: " + flags,
                    expectedPoints);
            ints.add(i);
            try {
        ...
        assertTest(flags.size() == 1, "add() doesn't block correctly!",
                expectedPoints);
        tiny.remove();
        try {
            Thread.sleep(100);
        ...
        for (int i = 0; i < 13; i++) {
            t = new Thread(() -> {
                tiny.add("Another Gibbon");
                flags.add("Another Gibbon");
            });
            t.start();
        }
        ...
        for (int i = 0; i < 13; i++) {
            ...
            assertTest(flags.size() == 1, "add() isn't blocking correctly!",
                    expectedPoints);
            tiny.remove();
            try {
                Thread.sleep(100);
        ...
        assertTest(doublySeenIds.size() == 0,
                "Two threads pulled the same integers (" + doublySeenIds +
                        ") out of the BlockingQueue!  That shouldn't happen!",
                expectedPoints);
         ...
         * System.out.println("Target time: " + targetMillis + " milliseconds. (Based
         * on your number of threads.)"); System.out.println( elapsed <= targetMillis ?
         * "You got it in the target time, great!" : "That took a long time!");
         */

        /*
         * int[][] targets = new int[][] {{10000, 20}, {13000, 15}, {16000, 10},
         * {20000, 5}};
         * 
         * int bonusPoints = 0;
         * 
         ...
         * bonusPoints = bonus;
         * System.out.println("Your code seems to be fast enough for a bonus!
         * (" + bonus + ") I would check with Kyle to make sure it runs fast enough on
         * his machine."); break;
         * }
         * }
        ...
        try {
            ...
            System.out.println(
                    "Just based on these tests, it looks like you'll earn " +
                            expectedPoints +
                            " points available.  (I will also have to look at your code to check the other parts that can't be tested!)");
        }
    }
}
